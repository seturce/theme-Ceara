...
?>
    <footer id='entity-owner' class="owner clearfix js-owner" ng-controller="ChangeOwnerController">
        ...
        <?php if ($this->isEditable() && $entity->canUser('changeOwner')) : ?>
            <h6 class="js-search js-include-editable" data-field-name='ownerId' data-emptytext="<?php i::esc_attr_e("Selecione um agente"); ?>" data-search-box-width="400px" data-search-box-placeholder="<?php i::esc_attr_e("Selecione um agente"); ?>" data-entity-controller="agent" data-search-result-template="#agent-search-result-template" data-selection-template="#agent-response-template" data-no-result-template="#agent-response-no-results-template" data-selection-format="changeOwner" data-auto-open="true" data-value="<?php echo $owner->id ?>" title="<?php i::esc_attr_e("Repassar propriedade"); ?>"><?php echo $owner->name ?></h6>
        <?php else : ?>
